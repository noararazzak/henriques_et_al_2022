...
directory_in_str = 'C:/Users/Noara/OneDrive/Summer 2022/Data/csv_cleaned/henriques_et_al'
...
class Property():
    def __init__(self, ticker, beta, std_dev, sharpe, jensen_alpha, total_trailing_return):
        self.ticker = ticker
        self.beta = beta
...
for file in os.listdir(directory):
    ...
    if filename.endswith(".csv"):
        csv_path = os.path.join(directory_in_str, filename)
        print(csv_path)
...
df_final = pd.DataFrame([{'Ticker': s.ticker, 'Beta': s.beta, 'Std Dev': s.std_dev, 'Sharpe': s.sharpe,
                          'Jensen': s.jensen_alpha, 'Trailing': s.total_trailing_return} for s in all_csv_property])
